...
df = pd.read_json('./output/task_vectors.jsonl', lines=True)
df.to_parquet('./output/task_vectors.parquet', index=False, compression='gzip')
print(df.head())
...
for i, row in job_table.iterrows():
    ...
    df_new = df_new.head(10)  # 取前10个最相似的任务

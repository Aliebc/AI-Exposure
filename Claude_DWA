...
MODEL_ID = os.getenv("METACHAT_MODEL_ID", "claude-3-5-haiku-20241022")
...
def get_ai_exposure_score(description: str) -> dict:
    """
    Assess AI exposure risk for a work activity description using MetaChat Claude API.
    ...
    if not API_KEY:
        result["reason"] = "MetaChat API key not configured"
        return result
    ...
    while retries <= MAX_RETRIES:
        try:
            ...
            if "429" in error_msg or "Too Many Requests" in error_msg:
                ...
                print(
                    f"Encountered rate limit (429), retry {retries}/{MAX_RETRIES}, waiting {wait_time:.2f} seconds...")
...
def main():
    ...
    print("DWA-AI Exposure Assessment Tool v2.0 (MetaChat Claude Version)")
    print(f"Using model: {MODEL_ID}")
    print("Using MetaChat Claude API")
    ...
    if run_full:
        ...
        save_results(final_results, "dwa_exposure_scores_full_claude.csv")
